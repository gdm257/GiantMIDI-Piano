"""This piano solo detection module is trained by Bochen Li in Feb. 2020, and then is cleaned up by Qiuqiang Kong in Jul. 2020."""
...
def wav2spec_mono(wav):
    spec = librosa.core.stft(
        y=wav, n_fft=N_FFT, hop_length=FRAME_HOP, win_length=FRAME_LEN, window=WIN, center='True', pad_mode='constant'
    )
...
def spec2wav_mono(mag, ang):
    ...
    wav = librosa.core.istft(stft_matrix=spec, hop_length=FRAME_HOP, win_length=FRAME_LEN, window=WIN, center='True')
    return wav
