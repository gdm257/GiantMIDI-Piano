...
def plot_piano_solo_p_r_f1(
    ...
    axs.xaxis.set_ticklabels([f'{e:.2f}' for e in thresholds], rotation=0)
    plt.tight_layout(pad=0, h_pad=0, w_pad=0)

    os.makedirs(os.path.dirname(out_fig_path), exist_ok=True)
    plt.savefig(out_fig_path)
    print(f'Write out to {out_fig_path}')
...
def piano_solo_meta_accuracy(
    ...
    print(f'Correct rate: {tp} / {tp + fp}, {precision}')
...
def piano_solo_performed_ratio(
    ...
    print(f'Performance ratio: {1. - sequenced_ratio:.3f}')
